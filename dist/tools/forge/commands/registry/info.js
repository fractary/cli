...
function createInfoCommand() {
    const cmd = new commander_1.Command('info');
    cmd
...
async function infoCommand(name, options) {
    ...
    if (!componentInfo) {
        console.error(chalk_1.default.red(`✗ Component '${name}' not found`));
        console.error();
        console.error(chalk_1.default.yellow('Try:'));
        console.error(chalk_1.default.yellow(`  • Check spelling: ${name}`));
        console.error(chalk_1.default.yellow('  • List installed: fractary forge list'));
        console.error(chalk_1.default.yellow(`  • Search remote: fractary forge search ${name}`));
        process.exit(1);
    }
    ...
    if (options.json) {
        ...
        (0, formatters_1.formatComponentInfo)({ ...componentInfo, availableVersions });
    }
    process.exit(0);
}
/**
 ...
 */
async function resolveComponent(name, options) {
    ...
    for (const scope of scopes) {
        try {
            ...
            const component = components.find((c) => c.name === name || c.plugin === name);
            if (component) {
                return {
                    name: component.name,
                    type: component.type,
            ...
            if (options.verbose && !options.json) {
                console.warn(chalk_1.default.dim(`Could not search ${scope}: ${error.message}`));
            }
        }
    }
    ...
    if (!options.local) {
        try {
            const resolved = await forge_1.Registry.resolver.resolve(name, options.type || 'agent', // Default to agent if no type specified
            { remoteOnly: true });
            if (resolved) {
                return {
                    name: resolved.name,
            ...
            if (options.verbose && !options.json) {
                console.warn(chalk_1.default.dim(`Remote resolution failed: ${error.message}`));
            }
        }
    }
    return null;
 ...
 */
async function fetchAvailableVersions(name, config) {
 ...
 * Handle info errors
 */
function handleInfoError(error, name) {
    const err = error;
    const hints = [];
    ...
    (0, formatters_1.formatError)(err, 'Info command failed', hints.length > 0 ? hints : undefined);
    process.exit(1);
}
// Export for use in index
exports.default = createInfoCommand;
