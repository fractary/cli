...
exports.createRegistryAddCommand = createRegistryAddCommand;
 ...
 */
function createRegistryAddCommand() {
 ...
 */
async function registryAddCommand(name, url, options) {
    // Validate name
    if (!isValidRegistryName(name)) {
        console.error(chalk_1.default.red('✗ Invalid registry name'));
        console.error();
        console.error(chalk_1.default.yellow('Registry name must:'));
    ...
    if (!options.force && !isValidUrl(url)) {
        console.error(chalk_1.default.red('✗ Invalid registry URL'));
        console.error();
        console.error(chalk_1.default.yellow('URL must:'));
        console.error(chalk_1.default.yellow('  • Start with https:// or file://'));
        console.error(chalk_1.default.yellow('  • Be a valid URL format'));
        console.error();
        console.error(chalk_1.default.dim('Use --force to skip validation'));
        process.exit(1);
    }
    // Validate type
    const type = options.type || 'manifest';
    if (type !== 'manifest' && type !== 'stockyard') {
        console.error(chalk_1.default.red(`✗ Invalid registry type: ${type}`));
        console.error();
        console.error(chalk_1.default.yellow('Valid types: manifest, stockyard'));
        process.exit(1);
    }
    // Parse options
    ...
    if (isNaN(priority) || priority < 1) {
        console.error(chalk_1.default.red('✗ Priority must be a positive number'));
        process.exit(1);
    }
    if (isNaN(cacheTtl) || cacheTtl < 0) {
        console.error(chalk_1.default.red('✗ Cache TTL must be a non-negative number'));
        process.exit(1);
    }
    ...
    if (config.registries.some((r) => r.name === name)) {
        console.error(chalk_1.default.red(`✗ Registry '${name}' already exists`));
        console.error();
        console.error(chalk_1.default.yellow('Try:'));
        console.error(chalk_1.default.yellow(`  • Use a different name`));
        console.error(chalk_1.default.yellow(`  • Remove existing: fractary forge registry remove ${name}`));
        process.exit(1);
    }
    ...
    if (!options.force && type === 'manifest') {
        if (options.verbose) {
            console.log(chalk_1.default.cyan('Validating registry accessibility...'));
        }
        const isAccessible = await testRegistryAccess(url);
        if (!isAccessible) {
            (0, formatters_1.formatWarning)(`Could not access registry at ${url}`);
            console.log();
            console.log(chalk_1.default.yellow('The registry might not be accessible or might not exist.'));
            console.log(chalk_1.default.yellow('Use --force to add anyway.'));
            process.exit(1);
        }
        if (options.verbose) {
            console.log(chalk_1.default.green('✓ Registry is accessible'));
            console.log();
        }
    }
    ...
    // Display success
    console.log();
    (0, formatters_1.formatSuccess)(`Registry '${name}' added successfully`);
    console.log();
    console.log(`  Name:     ${chalk_1.default.cyan(name)}`);
    console.log(`  Type:     ${chalk_1.default.cyan(type)}`);
    console.log(`  URL:      ${chalk_1.default.cyan(url)}`);
    console.log(`  Status:   ${newRegistry.enabled ? chalk_1.default.green('Enabled') : chalk_1.default.red('Disabled')}`);
    console.log(`  Priority: ${chalk_1.default.cyan(priority.toString())}`);
    console.log(`  Cache:    ${chalk_1.default.cyan(`${cacheTtl}s`)}`);
    console.log();
    if (config.registries.length > 1) {
        console.log(chalk_1.default.dim(`Total registries: ${config.registries.length} (sorted by priority)`));
    }
    process.exit(0);
}
/**
 ...
 */
async function testRegistryAccess(url) {
    try {
...
function handleAddError(error) {
    ...
    (0, formatters_1.formatError)(err, 'Registry add failed', hints.length > 0 ? hints : undefined);
    process.exit(1);
}
// Export for use in index
exports.default = createRegistryAddCommand;
//# sourceMappingURL=registry-add.js.map